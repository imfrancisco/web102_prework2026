...
// Calculate total number of individual contributions (backers)
...
const unfundedMessage = unfundedCount === 1 
    ? `There is ${unfundedCount} game that still needs your support to reach its funding goal!`
    : `There are ${unfundedCount} games that still need your support to reach their funding goals!`;

// create a new DOM element containing the template string and append it to the description container
const fundingInfo = document.createElement("p");
fundingInfo.innerHTML = `<strong>${unfundedMessage}</strong>`;
...
// sort games by pledged amount (highest first)
...
topGameName.textContent = `${topGame.name} ($${topGame.pledged.toLocaleString()})`;
firstGameContainer.appendChild(topGameName);

// do the same for the runner up item
const runnerUpName = document.createElement("p");
runnerUpName.textContent = `${runnerUpGame.name} ($${runnerUpGame.pledged.toLocaleString()})`;
